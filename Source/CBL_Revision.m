                       ...
                       deleted: (BOOL)deleted
{
    ...
    if (self) {
        ...
        _docID = docID.copy;
        Assert(!revID || [revID isKindOfClass: [CBL_RevID class]]);
        _revID = revID;
        _deleted = deleted;
    }
    return self;
...
- (CBL_MutableRevision*) mutableCopyWithDocID: (UU NSString*)docID revID: (UU CBL_RevID*)revID {
...
- (CBL_Revision*) revisionByAddingBasicMetadata {
    ...
    [props cbl_setID: self.docID rev: self.revID];
    if (_deleted)
        props[@"_deleted"] = @YES;
...
- (void) setDocID:(UU NSString *)docID revID: (UU CBL_RevID*)revID {
    Assert(docID);
    ...
    if (properties) {
        NSString* idProp = properties.cbl_id, *revProp = properties.cbl_revStr;
        if ((idProp && !$equal(idProp, _docID)) || (revProp && !$equal(revProp, revID.asString))) {
            NSMutableDictionary* nuProperties = [properties mutableCopy];
            [nuProperties setValue: docID forKey: @"_id"];
            [nuProperties setValue: revID.asString forKey: @"_rev"];
