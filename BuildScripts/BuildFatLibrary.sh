...
xcodebuild -configuration "${CONFIGURATION}" -target "${TARGET_NAME}" -sdk "${OTHER_SDK_TO_BUILD}" ${ACTION} RUN_CLANG_STATIC_ANALYZER=NO OBJROOT="${OBJROOT}/${TARGET_NAME}" BUILD_DIR="${BUILD_DIR}" BUILD_ROOT="${BUILD_ROOT}" CURRENT_PROJECT_VERSION="${CURRENT_PROJECT_VERSION}" CBL_VERSION_STRING="${CBL_VERSION_STRING}" CBL_SOURCE_REVISION="${CBL_SOURCE_REVISION}"  || exit 1
