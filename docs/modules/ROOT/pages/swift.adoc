...
include::{partialsdir}/swift/pre-release-instructions.adoc[]

=== Starter code

Open *ViewController.swift* in Xcode and copy the following code in the `viewDidLoad` method. This snippet demonstrates
how to run basic CRUD operations, a simple Query and running
bi-directional replications with Sync Gateway.

[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=getting-started]
----
...
In the next step, you will setup Sync Gateway. Before synchronizing documents to Sync Gateway you will need to disable App Transport Security (ATS) on iOS. App Transport Security is enabled by default since iOS 9 and enforces applications to use HTTPS exclusively. For this getting started guide, you will disable it but we recommend to enable it in production (and link:../guides/sync-gateway/configuring-ssl/index.html[enable HTTPS on Sync Gateway]). In the Xcode navigator, right-click on *Info.plist* and open it as a source file.
...
http://docs.couchbase.com/mobile/2.0/couchbase-lite-swift[Swift
SDK API References]
...
* Install the Couchbase Lite 2.0 framework in your project (see the link:swift.html#frameworks[installation options]). At this point, there will be many compiler warnings. Refer to the examples on this page to learn about the new API.
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=1x-attachment]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=new-database]
----

Just as before, the database will be created in a default location.
Alternatively, the
`Database(name: Strings, config: DatabaseConfiguration?)` initializer
can be used to provide specific options in the
http://docs.couchbase.com/mobile/2.0/couchbase-lite-swift/db022/Classes/DatabaseConfiguration.html[`DatabaseConfiguration`]
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=logging]
----
...
http://docs.couchbase.com/mobile/2.0/couchbase-lite-swift/db022/Classes/Database.html#/s:18CouchbaseLiteSwift8DatabaseC4copyySS8fromPath_SS02toD0AA0D13ConfigurationVSg10withConfigtKFZ[`Database.copy(fromPath:toDatabase:withConfig:)`]
method as shown below.

[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=prebuilt-database]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=initializer]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=update-document]
----
...
http://docs.couchbase.com/mobile/2.0/couchbase-lite-swift/db022/Classes/Document.html#/DictionaryProtocol[`property accessors`]
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=date-getter]
----
...
use the http://docs.couchbase.com/mobile/2.0/couchbase-lite-swift/db022/Classes/Document.html#/s:18CouchbaseLiteSwift8DocumentC8containsSbSS3key_tF[`Document.Contains(key: String)`] method.

A `Document` can be converted to a plain dictionary type. That's often useful to pass the
document contents as a plain object to another method.

[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=to-dictionary]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=batch]
----
...
It's also possible to register for document changes. The following example registers for
changes to the document with ID `user.john` and prints the `verified_account` property.

[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=document-listener]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=blob]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-select-meta]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-select-all]
----
...
The http://docs.couchbase.com/mobile/2.0/couchbase-lite-swift/db022/Classes/Expression.html#/Comparison%20Operators[comparison operators] can be used in the WHERE statement to specify on which
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-where]
----

==== Collection Operators

http://docs.couchbase.com/mobile/2.0/couchbase-lite-swift/db022/Classes/Expression.html#/Collection%20operators:[Collection
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-collection-operator]
----

==== Like Operator

The
http://docs.couchbase.com/mobile/2.0/couchbase-lite-swift/db022/Classes/Expression.html#/s:18CouchbaseLiteSwift10ExpressionC4likeACypF[`like`]
operator can be used for string matching. Use the
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-like-operator]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-like-operator-wildcard-match]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-like-operator-wildcard-character-match]
----

==== Regex Operator

The
http://docs.couchbase.com/mobile/2.0/couchbase-lite-swift/db022/Classes/Expression.html#/s:18CouchbaseLiteSwift10ExpressionC5regexACypF[`regex`]
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-regex-operator]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-join]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-groupby]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-orderby]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=query-index]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=live-query]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=stop-live-query]
----

== Full-Text Search

To run a full-text search (FTS) query, you must have created a full-text
index on the expression being matched. Unlike queries, the index
is not optional. The following example inserts documents and creates an
FTS index on the `name` property.

[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=fts-index]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=fts-query]
----
...
http://docs.couchbase.com/mobile/2.0/couchbase-lite-swift/db022/Classes/ReplicatorConfiguration.html[`ReplicatorConfiguration`]
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=replication]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=replication-logging]
----
...
To authenticate with Sync Gateway, an associated user must first be created. Sync Gateway users can be created through the link:../references/sync-gateway/admin-rest-api/index.html?v=2.0#/user/post__db___user_[`POST /{db}/_user`] endpoint on the Admin REST API. Provided that the user exists on Sync Gateway, there are two ways to authenticate from a Couchbase Lite client: Basic Authentication or Session Authentication.
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=basic-authentication]
----

==== Session Authentication

Session authentication is another way to authenticate with Sync Gateway. A user session must first be created through the link:../references/sync-gateway/rest-api/index.html#/session/post__db___session[`POST /{db}/_session`] endpoint on the Public REST API. The HTTP response contains a session ID which can then be used to authenticate as the user it was created for. The following example initiates a one-shot replication with the session ID that is returned from the `POST /{db}/_session` endpoint.

[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=session-authentication]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=replication-status]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=replication-error-handling]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=database-replica]
----
...
[source,swift]
----
include::{examplesdir}/SampleCodeTest.swift[tags=certificate-pinning]
----
