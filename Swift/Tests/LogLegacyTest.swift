...
/// Custom logger (deprecated `Logger` protocol) used by the legacy logging tests.
...
/// Tests for the deprecated logging API (Database.log / ConsoleLogger / FileLogger /
/// custom Logger), restored alongside the new LogSinks API. The class is marked
/// deprecated so the deprecated-API calls below don't emit warnings.
@available(*, deprecated)
class LogLegacyTest: CBLTestCase {
    ...
    override func setUp() {
        super.setUp()
        let folderName = "LogLegacyTestLogs_\(Int.random(in: 1...1000))"
