...
class PredictiveQueryWithCoreMLTest: CBLTestCase {
    ...
    func model(name: String, mustExist: Bool = true) throws -> CoreMLPredictiveModel? {
        guard let mlmodel = try coreMLModel(name: name, mustExist: mustExist) else {
            return nil
        }
        
        let modelName = (name as NSString).lastPathComponent
        return CoreMLPredictiveModel(name: modelName, model: mlmodel)
    }
    ...
    func testModelProperties() throws {
        ...
        XCTAssertEqual(model.name, "MarsHabitatPricer")
