...
class LogTest: CBLTestCase {
    ...
    var fileBackup: FileLogSink?
    var consoleBackup: ConsoleLogSink?
    ...
    override func setUp() {
        ...
        fileBackup = LogSinks.file
        consoleBackup = LogSinks.console
    }
    
    override func tearDown() {
        try? FileManager.default.removeItem(atPath: logFileDirectory)
        LogSinks.file = fileBackup
        LogSinks.console = consoleBackup
        LogSinks.custom = nil
    ...
    func testCustomLoggingLevels() throws {
        ...
        for i in (1...5).reversed() {
            LogSinks.custom = CustomLogSink(level: LogLevel(rawValue: UInt8(i))!, logSink: customLogger)
            customLogger.reset()
    ...
    func testFileLoggingLevels() throws {
        for i in (1...5).reversed() {
            LogSinks.file =
            FileLogSink(level: LogLevel(rawValue: UInt8(i))!, directory: logFileDirectory, usePlainText: true)
