    ...
    func testCustomLogSinkDomains() throws {
        ...
        for i in 0..<domains.count {
            ...
            for j in 0..<domains.count {
                Log._log(domain: LogDomain(rawValue: UInt(domains[j].rawValue))!, level: .verbose, message: names[j])
            }
        ...
        for i in 0..<domains.count {
            Log._log(domain: LogDomain(rawValue: UInt(domains[i].rawValue))!, level: .verbose, message: names[i])
        }
