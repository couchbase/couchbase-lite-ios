...
@synthesize name=_name, model=_model;
...
- (void) setupVisionModel {
    ...
    // See https://developer.apple.com/documentation/vision/vncoremlrequest for info:
    if (inputSpecs.count == 1 && inputSpecs.allValues[0].type == MLFeatureTypeImage) {
        ...
        if (!_vnModel) {
            CBLWarn(Query, @"Cannot create vision model from the model %@ : %@. "
                            "The regular MLModel will be used instead.", _name, error);
            return;
        }
...
- (void) register {
    [CBLDatabase.prediction registerModel: self withName: _name];
}


- (void) unregister {
    [CBLDatabase.prediction unregisterModelWithName: _name];
}
...
- (CBLDictionary*) predictionResultFromObservations: (NSArray*)observations {
    ...
    if (predictedFeatureName) {
        ...
        // Note: Only one output is supported now as there are no obvious ways to map result
        // observations to the outputs of the CoreML Model.
        id value = nil;
        MLModelDescription* desc = _model.modelDescription;
