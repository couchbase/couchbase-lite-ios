...
@synthesize name=_name, scope=_scope, c4col=_c4col, db=_db;
...
- (uint64_t) count {
    CBL_LOCK(_db) {
        return _c4col != nullptr ? c4coll_getDocumentCount(_c4col) : 0;
    }
}
...
- (BOOL) saveDocument: (CBLDocument*)document
   ...
   concurrencyControl: (CBLConcurrencyControl)concurrencyControl
    ...
    CBL_LOCK(_db) {
        if ([_db isClosedLocked]) {
            if (outError)
                *outError = CBLCollectionErrorNotOpen;
            return NO;
        }
        
        if (![_db prepareDocument: document error: outError])
            return NO;
