...
//  Couchbase Lite
//
//  Source: https://github.com/couchbase/couchbase-lite-ios/blob/master/Source/CBLSymmetricKey.m
...
- (instancetype) init {
    ...
    if (SecRandomCopyBytes(kSecRandomDefault, keyData.length, keyData.mutableBytes) != 0)
        return nil;
...
- (CBLCryptorBlock) createEncryptor {
    Header header;
    if (SecRandomCopyBytes(kSecRandomDefault, kIVSize, header.iv) != 0)
        return nil;
...
static BOOL readFully(NSInputStream* in, void* dst, size_t len) {
    ...
    for (size_t bytesRead = 0; bytesRead < len; bytesRead += n) {
        ...
        if (n <= 0) {
            CBLWarn(Default, @"SymmetricKey: readFully failed, error=%@", in.streamError.my_compactDescription);
            return NO;
        }
    }
    return YES;
...
static BOOL writeFully(NSOutputStream* out, const void* src, size_t len) {
    ...
    for (size_t bytesWritten = 0; bytesWritten < len; bytesWritten += n) {
        ...
        if (n <= 0) {
            CBLWarn(Default, @"SymmetricKey: writeFully failed, error=%@", out.streamError.my_compactDescription);
            return NO;
        }
    }
    return YES;
...
- (NSInputStream*) decryptStream: (NSInputStream*)encryptedStream {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        if (!decryptStreamSync(encryptedStream, writer, keyData))
            CBLWarn(Default, @"CBLSymmetricKey: decryptStream failed (bad input?)");
        [writer close];
    });
