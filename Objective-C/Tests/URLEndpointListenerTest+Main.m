...
//  CBL_ObjC_Tests
//
//  Created by Jayahari Vavachan on 4/12/22.
...
- (void) validateActiveReplicationsAndURLEndpointListener: (BOOL)isDeleteDBs {
    ...
    // start listener
    [self listenWithTLS: YES auth: nil];
...
- (void) testBusyPort {
    [self listenWithTLS: NO auth: nil];
...
- (void) testNonTLSNullListenerAuthenticator {
    ...
    Listener* listener = [self listenWithTLS: NO auth: nil];
    AssertNil(listener.tlsIdentity);
...
- (void) testMultipleReplicatorsToListener {
    ...
    [self listenWithTLS: YES auth: nil]; // writable listener
...
- (void) testReplicatorServerCertificate {
    ...
    Listener* listener = [self listenWithTLS: YES auth: nil];
