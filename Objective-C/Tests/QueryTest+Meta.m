...
- (void) testExpiryLessThanDate {
    ...
    NSTimeInterval future = [expiryDate dateByAddingTimeInterval: 2].timeIntervalSince1970 * 1000;
    CBLQuery* q = [CBLQueryBuilder select: @[kDOCID]
                                     from: [CBLQueryDataSource database: self.db]
                                    where: [[CBLQueryMeta expiration]
...
//    AssertEqual([[rs allObjects] count], 1u);
    ...
    rs = [q execute: &error];
    AssertNil(error);
    NSLog(@">>>----- query compare INTEGER and long long %lu", [[rs allObjects] count]);

    q = [self.db createQuery: @"select meta(testdb).id from _default as testdb where meta(testdb).expiration > 1.123" error: &error];
    AssertNotNil(q);
    rs = [q execute: &error];
    AssertNil(error);
    NSLog(@">>>----- query with direct N1QL %lu", [[rs allObjects] count]);
