...
- (void) testQueryResult {
    ...
    CBLQuery* q = [CBLQueryBuilder select: @[S_FNAME, S_LNAME, S_EMAIL, S_ADDRESS, S_CODE,
                                             S_YEAR, S_ID, S_SCORE, S_IS_FULLTIME, S_START_DATE]
                                     from: [CBLQueryDataSource database: self.db]];
    
    NSSet* keys = [NSSet setWithObjects: @"lastname", @"email", @"address", @"firstname", @"code",
                   @"year", @"id", @"score", @"isFullTime", @"startDate", nil];
    uint64_t numRows = [self verifyQuery: q randomAccess: YES
                                    test: ^(uint64_t n, CBLQueryResult* r)
    {
        AssertEqual(r.count, 10u);
