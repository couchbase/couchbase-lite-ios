...
- (CBLCoreMLPredictiveModel*) model: (NSString*)name mustExist: (BOOL)mustExist {
    ...
    NSString* modelName = [name lastPathComponent];
    return [[CBLCoreMLPredictiveModel alloc] initWithName: modelName model: mlmodel];
}
...
- (void) testModelProperties {
    ...
    AssertEqualObjects(model.name, @"MarsHabitatPricer");
    AssertNotNil(model.model);
}
...
- (void) testMarsHabitatPricerModel {
    ...
    CBLQuery *q = [CBLQueryBuilder select: @[SEL_PROP(@"expected_price"),
                                             SEL_EXPR(PREDICTION(model.name, EXPR_VAL(input)))]
                                     from: kDATA_SRC_DB];
...
- (void) testInvalidInput {
    ...
    NSArray* tests =
    @[
      @[@1.0, @"1", @750], // solarPanels, greenhouses, size, rounded expected_price
      @[[NSNull null], @2, @1000],
      @[@3.0, @5, @2000, @11635]
      ];
    [self createMarsHabitatPricerModelDocuments: tests];
...
- (void) testMobileNetModel {
    ...
    CBLQuery *q = [CBLQueryBuilder select: @[SEL_EXPR(PREDICTION(model.name, EXPR_VAL(input)))]
                                     from: kDATA_SRC_DB];
    uint64_t numRows = [self verifyQuery: q randomAccess: NO
                                    test: ^(uint64_t n, CBLQueryResult *r)
    {
...
- (void) testOpenFaceModel {
    ...
    CBLQueryExpression* vector2 = PREDICTION_VALUE(model.name, EXPR_VAL(input), @"output");
...
- (void) testBasicDataConversion {
    ...
    // value, MLFeatureType, MLFeatureSubType, non-null-returned
    NSArray *tests = @[@[@(9),      @(MLFeatureTypeInt64),      @(MLFeatureTypeInvalid), @(YES)],
                       @[@"nine",   @(MLFeatureTypeInt64),      @(MLFeatureTypeInvalid), @(NO)],
                       @[@(20.0),   @(MLFeatureTypeDouble),     @(MLFeatureTypeInvalid), @(YES)],
                       @[@"twenty", @(MLFeatureTypeDouble),     @(MLFeatureTypeInvalid), @(NO)],
                       @[@"string", @(MLFeatureTypeString),     @(MLFeatureTypeInvalid), @(YES)],
                       @[@(1),      @(MLFeatureTypeString),     @(MLFeatureTypeInvalid), @(NO)],
                       @[dict,      @(MLFeatureTypeDictionary), @(MLFeatureTypeInvalid), @(YES)],
                       @[@"dict",   @(MLFeatureTypeDictionary), @(MLFeatureTypeInvalid), @(NO)],
                       ];
    
    for (NSArray* test in tests) {
        MLFeatureType type = [test[1] integerValue];
        BOOL notNull = [test[3] boolValue];
